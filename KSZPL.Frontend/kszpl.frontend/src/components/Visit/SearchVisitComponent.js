...
class SearchVisitComponent extends Component {
  ...
  componentDidMount() {
    axios.get(BASE_URL + "visit/createvisit").then(response => {
      this.setState({
      ...
      return this.state.visits.map(visit => (
        <tr>
          ...
          <td>
            ...
            <Button
              variant="info"
              onClick={() => this.OnClickMoreInfo(visit.id)}
            >
              Więcej
            </Button>{" "}
          </td>
          <td>
            ...
            <Button
              variant="primary"
              onClick={() => this.OnClickAddRecipe(visit.id)}
            >
              Dodaj receptę
            </Button>{" "}
          </td>
  ...
  findVisit = () => {
    ...
    axios
      .get(
        BASE_URL +
          `visit/find/${this.state.patientId}/${
            this.state.doctorId
          }/${this.parseDate(this.state.dateVisit)}`
      )
      .then(response => {
        debugger;
      ...
      <div>
        <Card>
          <Card.Header className="color-khaki">
            <Card.Title>Wyszukaj wizytę</Card.Title>
          </Card.Header>
          <Card.Body>
            <Row>
              <Col sm={2}> </Col>
